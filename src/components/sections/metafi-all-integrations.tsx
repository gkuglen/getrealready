...
const SERVICES: Service[] = [
  {
    ...
    description: 'Fast, reliable make-ready services to minimize vacancy and get units back on market quickly.',
    items: ['Paint, flooring, repairs', '3–7 day turnaround', 'Fixed pricing options'],
    icon: '/images/homepage/integrations/shopify.svg',
  },
  {
    name: 'Value-Add Renovations',
    description: 'Targeted upgrades designed to improve rent performance and property value.',
    items: ['Kitchen and bath upgrades', 'Flooring and lighting packages', 'ROI-driven recommendations'],
  ...
  },
  {
    ...
    description: 'Simple, reliable maintenance to keep your property performing.',
    items: ['On-demand repairs', 'Preventative maintenance', 'Optional monthly service plans'],
  ...
  return (
    <section id="grr-services" className="bg-background px-6 lg:px-0">
      <div className="container px-0 py-16 sm:py-20 md:px-6 md:py-24">
        <p className="text-tagline mb-4 text-center text-sm sm:text-base">Services</p>
          ...
          {SERVICES.map((svc) => (
            <li key={svc.name} className="h-full">
              <article className="bg-card border-border-light shadow-light h-full rounded-[16px] border p-4">
                ...
                <h3 className="text-foreground mt-4 text-2xl font-medium">{svc.name}</h3>
                <p className="text-muted-foreground mt-2 text-[18px]">{svc.description}</p>
                ...
                {svc.note && (
                  <p className="text-muted-foreground mt-4 text-sm italic">{svc.note}</p>
                )}
              </article>
            </li>
          ))}
